...
Section build_fan.
  ...
  (** Below, f : X → list X describes a finitary FAN
  ...
  (* A fan for f is a ltree X such that at every node ⟨x|l⟩ₗ 
     the list of labels of each son (ie map ltree_node l) is
     exactly f x *) 
  ...
  (* Notice that a fan may not exists, but if it exists, all its nodes are R-accessible *)
  ...
  Fact fan_uniq t₁ t₂ : fan t₁ → fan t₂ → ltree_node t₁ = ltree_node t₂ → t₁ = t₂.
  Proof.
    intros H1 H2; revert H1 t₂ H2.
  ...
  (* A fan_at x is a fan with root node labeled by x *)
  Definition fan_at x t := ltree_node t = x ∧ fan t.

  (* The fan_at x is unique *)
  Fact fan_at_uniq x t₁ t₂ : fan_at x t₁ → fan_at x t₂ → t₁ = t₂.
  Proof.
    intros (H1 & H2) (<- & H4).
  ...
  (* The irreductible elements generated by x are collected
     in the leaves of a fan_at x *)
  Definition irred x := ltree_leaves (proj1_sig (FAN x)).

  (* This is the first fixpoint equation for irred *)
  ...
  (* This is the second fixpoint equation for irred *)
